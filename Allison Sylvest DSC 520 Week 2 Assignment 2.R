...
## Load the file `person.csv` to `person_df1` using `read.csv`
## Examine the structure of `person_df1` using `str()`
person_df1 <- read.csv(file='person.csv')
str(person_df1)
...
scores_df <- read.csv(file = 'scores.csv', stringsAsFactors = TRUE)
summary(scores_df)
...
## list the worksheets from the file `G04ResultsDetail2004-11-02.xls`
...
voter_turnout_df1 <-read_excel('G04ResultsDetail2004-11-02.xls', "Voter Turnout", skip = 1)
str(voter_turnout_df1)

## Using the `read_excel()` function, read the Voter Turnout sheet
## from `G04ResultsDetail2004-11-02.xls`
...
voter_turnout_df2 <- read_excel('G04ResultsDetail2004-11-02.xls', "Voter Turnout", skip = 2, col_names = c("ward_precint", "ballots_cast", "registered_voters", "voter_turnout"))
...
## Create a database connection to `example.db` using the dbConnect() function
